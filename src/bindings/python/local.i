 ...
 * 1. wraps std::cout
 *
 *    xos = libtsb.XMLOutputStream(libtsb.cout)
 *
 * 2. wraps std::cerr
 *
 *    d = libtsb.readTSB("foo.xml")
 *    if d.getNumErrors() > 0 :
 *       d.printErrors(libtsb.cerr)
 *    
 *
 * 3. wraps std::ostringstream
 *
 *    oss = libtsb.ostringstream()
 *    xos = libtsb.XMLOutputStream(oss)
 *    ...
 *    libtsb.endl(oss)
 *    s = oss.str()
 * 
...
/**
 * Convert an TSBBase object to a string.
 *
%extend TSBBase
{
  %pythoncode
  {
    def __str__(self):
      ...
      Note that the string will not be a complete TSB document.
      """
 ...
 */
%typemap(out) TSBBase*, SedSimulation*, SedTask*, SedOutput*, SedChange*, SedRange*
{
...
%{
        if type(args[1]) == type(True): return _libtsb.XMLOutputStream_writeAttributeBool(self, *args)
%}
#else 
%feature("pythonprepend")
XMLOutputStream::writeAttribute
%{
        if type(args[2]) == type(True): return _libtsb.XMLOutputStream_writeAttributeBool(*args)
%}
#endif

/**
 * Add an equality operator to TSBBase.  All subclasses of TSBBase
 ...
 * example, each TSBBase that is appended to a TSBListOf is subsequently
 * owned by that TSBListOf.
 */
...
TAKEOVER_OWNERSHIP(TSBListOf::appendAndOwn(TSBBase*),item)
...
/**
 *
 * Wraps the SedConstructorException class (C++ exception defined by libTSB) 
 ...
 * --------------------------------------
 *  try:
 *    s = libtsb.CompartmentType(level,version)
 *  except ValueError, inst:
 *    errmsg = inst.args[0]
 * --------------------------------------
 */

%ignore TSBConstructorException;
...
/**
 *
 * Wraps the XMLConstructorException class (C++ exception defined by libTSB) 
 ...
 * --------------------------------------
 *  try:
 *    s = libtsb.XMLAttributes(invalid arguments)
 *  except ValueError, inst:
 *    errmsg = inst.args[0]
 * --------------------------------------
 */
...
def conditional_abspath (filename):
  ...
  encapsulates the appropriate logic.  It is used by readTSB() and
  SedReader.readTSB().
  """
 ...
 */
%feature("shadow")
SedReader::readTSB(const std::string&)
%{
  def readTSB(*args):
    """
    readTSB(self, string filename) -> TSBDocument

    Reads an TSB document from a file.

    This method is identical to readTSBFromFile().

    If the file named 'filename' does not exist or its content is not
    valid TSB, one or more errors will be logged with the TSBDocument
    object returned by this method.  Callers can use the methods on
    TSBDocument such as TSBDocument.getNumErrors() and
    TSBDocument.getError() to get the errors.  The object returned by
    TSBDocument.getError() is an SedError object, and it has methods to
    ...
    for these situations using a program fragment such as the following:
     ...
     reader = SedReader()
     doc    = reader.readTSB(filename)

     if doc.getNumErrors() > 0:
       if doc.getError(0).getErrorId() == libtsb.XMLFileUnreadable:
         # Handle case of unreadable file here.
       elif doc.getError(0).getErrorId() == libtsb.XMLFileOperationError:
    ...
    To read a gzip/zip file, libTSB needs to be configured and linked with
    ...
    default configurations for libTSB.)  Errors about unreadable files
    will be logged if a compressed filename is given and libTSB was not
    linked with the corresponding required library.

    Parameter 'filename is the name or full pathname of the file to be
    read.

    Returns a pointer to the TSBDocument created from the TSB content.

    See also SedError.

    Note:

    LibTSB versions 2.x and later versions behave differently in
    error handling in several respects.  One difference is how early some
    errors are caught and whether libTSB continues processing a file in
    the face of some early errors.  In general, libTSB versions after 2.x
    stop parsing TSB inputs sooner than libTSB version 2.x in the face
    of XML errors, because the errors may invalidate any further TSB
    content.  For example, a missing XML declaration at the beginning of
    the file was ignored by libTSB 2.x but in version 3.x and later, it
    will cause libTSB to stop parsing the rest of the input altogether.
    ...
    libTSB.  The XML parsers themselves behave differently in their error
    reporting, and sometimes libTSB has to resort to the lowest common
    denominator.
    """
    args_copy    = list(args)
    args_copy[1] = conditional_abspath(args[1])
    return _libtsb.SedReader_readTSB(*args_copy)
%}

%feature("shadow")
SedReader::readTSBFromFile(const std::string&)
%{
  def readTSBFromFile(*args):
    """
    readTSBFromFile(self, string filename) -> TSBDocument
...
/**
 * Since we cannot seem to "shadow" readTSB() (maybe because it's
 * not a method of some object, but rather a top-level function, we
 * employ the following HACK: Tell SWIG to ignore readTSB and just
 * define it in terms of SedReader.readTSB().  This is less than
 * ideal, because the libTSB C/C++ core does essentially the same
 * thing, so now we're repeating ourselves.
 */

%ignore readTSB(const char*);
 ...
 *  - List* TSBBase::getCVTerms()
 *  - List* TSBNamespaces::getSupportedNamespaces()
 *
 ...
 *  - CVTermList       TSBBase.getCVTerms()
 *  - TSBNamespacesList TSBNamespaces::getSupportedNamespaces()
 *
 */
...
%{
  def getSupportedNamespaces(self):
    """
    getSupportedNamespaces(self) -> TSBNamespaceList

    Get the List of supported TSBNamespaces for this 
    version of LibTSB.

    Returns the supported list of TSBNamespaces.
          

    """
    return _libtsb.TSBNamespaces_getSupportedNamespaces(self)
%}

%typemap(out) List* TSBNamespaces::getSupportedNamespaces
{
  ListWrapper<TSBNamespaces> *listw = ($1 != 0) ? new ListWrapper<TSBNamespaces>($1) : 0;
  $result = SWIG_NewPointerObj(SWIG_as_voidptr(listw), 
#if SWIG_VERSION > 0x010333
                               SWIGTYPE_p_ListWrapperT_TSBNamespaces_t, 
#else
                               SWIGTYPE_p_ListWrapperTTSBNamespaces_t, 
#endif
                               SWIG_POINTER_OWN |  0 );
}
