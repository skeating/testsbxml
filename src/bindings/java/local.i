...
* In JDK 1.4.2, libtsb's *::clone() methods can't override 
...
* is required for libTSB Java binding, and thus we don't have to use
...
* TSBConstructorException (Java class) which is derived from
...
%typemap(javabase) TSBConstructorException "java.lang.IllegalArgumentException";
%typemap(javacode) TSBConstructorException 
%{
	protected TSBConstructorException(long cPtr, boolean cMemoryOwn, String v)
	{
		super(v);
	...
	/**
* Exception thrown by libTSB object constructors if something prevents
* the object from being properly created.
*
* An example of what can cause this exception to be thrown is attempting
* to create an TSB component with an invalid TSB Level/Version
* combination.
*/
	public TSBConstructorException(String v)
	{
		this(libtsbJNI.new_TSBConstructorException__SWIG_0(), true, v);
	}
	%}

%define SEDMLCONSTRUCTOR_EXCEPTION(SBASE_CLASS_NAME)
%javaexception("org.tsb.libtsb.TSBConstructorException") SBASE_CLASS_NAME
%{
	try {
		$action
	}
		...
		jenv->ExceptionClear();
		jclass clazz = jenv->FindClass("org/tsb/libtsb/TSBConstructorException");
		if (clazz)
...
*/
	...
	{
		this(libtsbJNI.new_XMLConstructorException(), true, v);
	}
	%}
...
%javaexception("org.tsb.libtsb.XMLConstructorException") SBASE_CLASS_NAME
%{
	try {
		$action
	}
		...
		jenv->ExceptionClear();
		jclass clazz = jenv->FindClass("org/tsb/libtsb/XMLConstructorException");
		if (clazz)
		jenv->ThrowNew(clazz, e.what());
		return $null;
	}
...
* TSBErrorLog::getError(unsigned int).
* (XMLErrorLog is a derived class of TSBErrorLog)
* In JDK 1.4.2, "TSBError* TSBErrorLog::getError(unsigned int)"
...
/**
* Make most libTSB constants (e.g. TSBTypecodes) Java compile-time
...
/**
* @return the most specific Java object possible for the given TSBBase
* object.
*/
...
* @internal
*/
	public static TSBBase DowncastTSBBase(long cPtr, boolean owner)
	{
...
*/
%typemap("javaout") TSBBase*
{
	return libtsb.DowncastTSBBase($jnicall, $owner);
}
...
/**
*
* Overrides the 'equals' method for TSBBase subclasses and other classes
* in libtsb.
*
...
* <p>
* Because the Java methods for libTSB are actually wrappers around code
* implemented in C++ and C, certain operations will not behave as
* expected.  Equality comparison is one such case.  An instance of a
* libTSB object class is actually a <em>proxy object</em>
...
* result is an assessment of whether two libTSB Java objects are truly 
...
SWIGJAVA_EQUALS(TSBBase)
SWIGJAVA_EQUALS(TSBReader)
SWIGJAVA_EQUALS(TSBWriter)
...
/**
* Part of libTSB methods takeover ownership of passed-in objects, so we need
* to make sure SWIG disowns the object.
* (Most libTSB methods don't takeover ownership since 3.0.0)
*
*/
...
*/
%typemap(javain) TSBBase*       item "TSBBase.getCPtrAndDisown($javainput)";
%typemap(javain) const TSBBase* item "TSBBase.getCPtr($javainput)";
...
* instead of UTF-8 because file I/O APIs internally used in libTSB 
* requires an ANSI CP encoded string for the given filename.
*  
*  1) TSBReader::readTSB(const std::string& filename)
*  2) readTSB(const char* filename)
*  3) TSBWriter::writeTSB(TSBDocument*, const std::string& filename)
*  4) writeTSB(TSBDocument*, const char* filename)
*/
...
* hide this from the end-user by making TSBReader.readTSB() and
* readTSB() always compute an absolute path and filename.
*/
...
%typemap("javain") const std::string& filename "libtsb.getAbsolutePath($javainput)";
%typemap("javain") const char* filename        "libtsb.getAbsolutePath($javainput)";
...
* 1. wraps std::cout
*
*    XMLOutputStream xos = new XMLOutputStream(libtsb.cout);
*
* 2. wraps std::cerr
*
*    TSBDocument d = libtsb.readTSB("foo.xml");
*    if ( d.getNumErrors() > 0) {
...
* <p>
* A few libTSB methods accept an argument for indicating where to send
* text string output.  An example is the {@link
* TSBDocument#printErrors} method.  However, the methods use C++ style
* streams and not Java stream objects.  The OStream object class in the
* libTSB Java interface provides a wrapper for the underlying C++
...
* TSBDocument document = libtsb.readTSB("somefile.xml");
* if (document.getNumErrors() > 0)
* {
*     document.printErrors(libtsb.cout);
...
* if (document.getNumErrors() > 0)
* {
*     document.printErrors(libtsb.cerr);
*     println("Please correct the above problems first.");
*     System.exit(1);
* }</div>
* <p>
...
* if (document.getNumErrors() > 0)
* {
*     document.printErrors(libtsb.clog);
*     println("Please correct the above problems first.");
*     System.exit(1);
* }</div>
* <p>
...
*  - TSBNamespacseList TSBNamespaces.getSupportedNamespaces()
*
*/
...
LIST_WRAPPER(TSBBase::getTSBListOfAllElements,TSBBase)
