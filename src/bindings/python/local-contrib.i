 ...
 * @brief   Contributed class that makes using libTSB from python more convenient
 *          for the time being this only works for python 2.x, but won't break python 3
 * @author  Gordon Ball
 * 
 *<!---------------------------------------------------------------------------
 * This file is part of libTSB.  Please visit http://tsb.org for more
 * information about TSB, and the latest version of libTSB.
 *
 ...
 * and also available online as http://tsb.org/software/libtsb/license.html
 *----------------------------------------------------------------------- -->*/

 
/**
 * Metaclass to convert getX/setX methods into python properties.
 * This is attached to TSBBase and automatically applies for all classes
    ...
    def __new__(cls, classname, bases, classdict):
        ...
        for name in get_methods | set_methods:
            ...
            if name in get_methods:
                ...
                #test if function is "return _libtsb.CLASS_getNAME(__args__)"
                if getter.func_code.co_names == ('_libtsb', cname):
                    getter = getattr(_libtsb, cname)
            ...
            if name in set_methods:
                ...
                if setter.func_code.co_names == ('_libtsb', cname):
                    setter = getattr(_libtsb, cname)
            ...
            if 'unset' + name in classdict:
                ...
                if deleter.func_code.co_names == ('_libtsb', cname):
                    deleter = getattr(_libtsb, cname)
